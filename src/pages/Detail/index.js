import React from "react";
import { useHistory } from "react-router-dom";

export default function Detail() {
  const history = useHistory();
  return (
    <>
      {/* <!-- Navbar --> */}
      <nav className="navbar navbar-expand-lg navbar-dark bg-primary shadow sm fixed-top">
        <div className="container">
          <a className="navbar-brand" href="*">
            EL Market
          </a>
          <button
            className="navbar-toggler"
            type="button"
            data-bs-toggle="collapse"
            data-bs-target="#navbarNav"
            aria-controls="navbarNav"
            aria-expanded="false"
            aria-label="Toggle navigation"
          >
            <span className="navbar-toggler-icon"></span>
          </button>
          <div className="collapse navbar-collapse" id="navbarNav">
            <ul className="navbar-nav ms-auto">
              <li className="nav-item">
                <a
                  className="nav-link active"
                  aria-current="page"
                  href="#back"
                  onClick={() => history.goBack()}
                >
                  Kembali
                </a>
              </li>
              <li className="nav-item">
                <a className="nav-link" aria-current="page" href="#infoproduct">
                  Info Product
                </a>
              </li>
              <li className="nav-item">
                <a className="nav-link" href="#ulasan">
                  Ulasan
                </a>
              </li>
              <li className="nav-item">
                <a className="nav-link" href="#recomendasi">
                  Rekomendasi
                </a>
              </li>
              {/* <li className="nav-item">
                <a className="nav-link" href="*"></a>
              </li> */}
            </ul>
          </div>
        </div>
      </nav>
      {/* // <!-- Akhir Navbar --> */}

      {/* <!-- Jumbotron --> */}
      <section className="jumbotron text-center mb-5 pt-5" id="infoproduct">
        <img
          src="data:image/jpeg;base64,"
          alt="gambar"
        />
        <p className="fw-bold fs-4">
          LAPTOP ASUS A412FA N5405U 8GB 1TB UHD GRAPHIC WIN10 SLIM
        </p>
        <p className="fw-bold fs-5 text-danger">Rp6.899.000</p>
        <hr className="my-4" />
      </section>
      {/* <!-- Akhir Jumbotron --> */}

      {/* <!-- Deskripsi--> */}
      <section id="deskripsi">
        <div className="container">
          <div className="row text-sm-between">
            <div className="col">
              <h4 className="fw-bold fs-6">
                Kondisi: Baru.
                <p> Berat: 4.000 Gram.</p> <p>Kategori: Laptop Consumer.</p>
                <p className="fw-bold">Spesifications :</p>
                <p>- Processor intel gold N5405U</p>
                <p>- RAM 8GB DDR4</p>
                <p>- Storage 1TB</p>
                <p>- OS Windows 10 Home</p>
                <p>- Graphics Card INTEL HD</p>
                <p>
                  - Display 14''" (16:9) LED backlit FHD (1920x1024)Glare, Ultra
                  Slim 200nits
                </p>
                <p>
                  - Ports 1x Microphone-in/Headphone-out jack, 1x USB 3.0 port,
                  1x HDMI, 1x MicroSD card reader, 1x USB 2.0 Port
                </p>
                <p>- Connectivity Integrated 802.11b/g/n + Bluetooth v4.0</p>
                <p>
                  - Includes 2 year warranty, VGA Web Camera, ASUS SonicMaster
                  Technology, Fingerprint
                </p>
              </h4>
            </div>
          </div>
        </div>
      </section>

      {/* <!-- Akhir Deskripsi--> */}

      {/* <!-- Button-->  */}
      <section id="button">
        <div className="container">
          <div className="row text-end">
            <div className="col">
              <button className="btn btn-primary btn-lg mx-4">Buy Now</button>
              <button className="btn btn-primary btn-lg">Wishlist</button>
            </div>
            <hr className="my-4" />
          </div>
        </div>
      </section>
      {/* <!-- Akhir Button-->  */}

      {/* <!-- Ulasan --> */}
      <section id="ulasan">
        <div className="container">
          <div className="row text-center">
            <div className="col text-center">
              <h2>Ulasan</h2>
            </div>
          </div>
          <div className="row justify-content-start">
            <div className="col mb-3 mt-3">
              <form>
                <div className="mb-3">
                  <p className="fst-normal fs-5">Krisnaars2@gmail.com</p>
                  <textarea className="form-control" id="pesan" rows="3">
                    Barang ini sangat bagus dan memuaskan
                  </textarea>
                </div>
              </form>
            </div>
          </div>
          <div className="row justify-content-start">
            <div className="col md-3 mt-3">
              <form>
                <div className="mb-3">
                  <p className="fst-normal fs-5">Bimaprada112@gmail.com</p>
                  <textarea className="form-control" id="pesan" rows="3">
                    Barang sangat sesuai ekspetasi dan cepat sampainya
                  </textarea>
                </div>
              </form>
            </div>
          </div>
          <div className="row justify-content-start">
            <div className="col mb-3 mt-3">
              <form>
                <div className="mb-3">
                  <p className="fst-normal fs-5">Jokopradipta001@gmail.com</p>
                  <textarea className="form-control" id="pesan" rows="3">
                    Kurang memuaskan, karna tidak sama dengan digambar
                  </textarea>
                </div>
              </form>
            </div>
          </div>
        </div>
      </section>
      {/* <!-- Akhir Ulasan --> */}

      {/* <!-- Recomendasi--> */}
      <section id="recomendasi">
        <div className="container">
          <div className="row text-center pt-5">
            <div className="col">
              <h2>Recomendasi</h2>
            </div>
          </div>
          <div className="row justify-content-center">
            <div className="col-md-2 mt-5">
              <div className="card">
                <img
                  src="img/satu detailpage.jpg"
                  className="card-img-top"
                  alt="baris pertama"
                />
                <div className="card-body">
                  <p className="card-text">Arduino Nano </p>
                </div>
              </div>
            </div>
            <div className="col-md-2 mt-5">
              <div className="card">
                <img
                  src="img/dua detailpage.jpg"
                  className="card-img-top"
                  alt="baris kedua"
                />
                <div className="card-body">
                  <p className="card-text">Arduino Due</p>
                </div>
              </div>
            </div>
            <div className="col-md-2 mt-5">
              <div className="card">
                <img
                  src="img/tiga detailpage.jpg"
                  className="card-img-top"
                  alt="baris ketiga"
                />
                <div className="card-body">
                  <p className="card-text">Arduino Mega</p>
                </div>
              </div>
            </div>
            <div className="col-md-2 mt-5">
              <div className="card">
                <img
                  src="img/empat detailpage.jpg"
                  className="card-img-top"
                  alt="baris keempat"
                />
                <div className="card-body">
                  <p className="card-text">Arduino Fio</p>
                </div>
              </div>
            </div>
            <div className="col-md-2 mt-5">
              <div className="card">
                <img
                  src="img/lima detailpage.jpg"
                  className="card-img-top"
                  alt="baris kelima"
                />
                <div className="card-body">
                  <p className="card-text">Arduino Lilypad</p>
                </div>
              </div>
            </div>
            <div className="col-md-2 mt-5">
              <div className="card">
                <img
                  src="img/enam detailpage.jpg"
                  className="card-img-top"
                  alt="baris keenam"
                />
                <div className="card-body">
                  <p className="card-text">Arduino Leonardo</p>
                </div>
              </div>
            </div>
          </div>
        </div>
      </section>

      {/* <!-- Akhir Recomendasi--> */}
    </>
  );
}
